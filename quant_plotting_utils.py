...
def pareto_plot(pareto_front_data, baseline_data, score_type='BB-IoU', filter_front=True, show_layer=None):
    ...
    if show_layer is not None:
        configurations = [config for config in configurations if show_layer in config]
    
    plt.rcParams['font.family'] = 'serif'
    markers = {"baseline": "X", "Energy": "o", "L1": "v", "PPCE": "p", "RRR*": "D"}
    colours = {"baseline": "#FFFFFF", "Energy": "#ED3E63", "L1": "#39D095", "PPCE": "#FECB5B", "RRR*": "#187FA8"}
    ...
    _, axes = plt.subplots(num_rows, num_cols, figsize=(18, 5 if show_layer is None else 2.5), dpi=300, sharex='col', sharey='row')
    axes = np.array(axes).reshape(-1)  # Ensure axes is a flat array for easy iteration
    ...
    for idx, (ax, config) in enumerate(zip(axes, configurations)):
        ...
        ax.set_xticks(np.arange(ax.get_xlim()[0] // 5 * 5, ax.get_xlim()[1], 5))
        ax.set_yticks(np.arange(ax.get_ylim()[0] // 10 * 10, ax.get_ylim()[1], 10))
        ...
        if idx % 3 == 0:
            ...
            ax.set_ylabel("EPG Score (%)") if score_type == "BB-Loc" else ax.set_ylabel("IoU Score (%)")
        ...
        if baseline_key in baseline_data:
            plot_baseline(ax, baseline_data[baseline_key], markers, colours, score_type)
        ...
        for loss_fn in markers.keys():
            if loss_fn != "baseline":
                ...
                if pareto_key in pareto_front_data:
                    ...
                    pareto_points = np.array([(model["F-Score"]*100, model["BB-Loc"]*100) for model in pareto_metrics])
                    if filter_front:
                    ...
                    if sorted_pareto_points.size > 0:
                        ...
                        ax.scatter(f_scores, bb_locs, label=loss_fn, marker=markers[loss_fn], color=colours[loss_fn], edgecolors='black', s=69, zorder=3)
                        ax.plot(f_scores, bb_locs, linestyle='--', color=colours[loss_fn], linewidth=2, zorder=1)

        # Add attribution method titles
        ax.text(0.01, 0.85, attribution_titles[idx % 3], horizontalalignment='left', verticalalignment='center', transform=ax.transAxes, color='blue', fontsize=14)
        ...
        ax.legend(handles, labels, loc='lower center', bbox_to_anchor=(0.5, 0.85), ncol=4, fontsize=9, frameon=True, framealpha=1)
...
def plot_dilations(plot_data, baseline_data, metric=None):
    ...
    if metric == "EPG":
        configurations = [config for config in configurations if config[1] == "EPG"]
    elif metric == "IoU":
        configurations = [config for config in configurations if config[1] == "IoU"]

    plt.rcParams['font.family'] = 'serif'
    markers = {"baseline": "X", "Energy": "o", "L1": "v"}
    colours = {"baseline": "#FFFFFF", "0%": "#11285E", "10%": "#2E7AB5", "25%": "#A0C8E0", "50%": "#F6FAFF"}
    ...
    _, axes = plt.subplots(nrows, ncols, figsize=(11, 4 if nrows == 2 else 2.5), dpi=300, sharex='col', sharey='row')
    ...
    for idx, (ax, config) in enumerate(zip(axes, configurations)):
        ...
        # Plot the baseline
        plot_baseline(ax, baseline_data[('bcos', 'BCos', 'Input')], markers, colours, score_type=score_type)
        ...
        for dilation in [0, 0.1, 0.25, 0.5]:
            ...
            if config_key in plot_data:
                ...
                pareto_points = np.array([(model["F-Score"]*100, model[score_type]*100) for model in pareto_metrics])
                sorted_pareto_points = sort_pareto_front(pareto_points)
                if sorted_pareto_points.size > 0:
                    f_scores, loc_scores = zip(*sorted_pareto_points)
                    colour_key = f"{int(dilation*100)}%"
                    ax.scatter(f_scores, loc_scores, label=colour_key, marker=markers[loss_fn], color=colours[colour_key], edgecolors='black', s=69, zorder=3)
        ...
        # Add legend
        handles, labels = ax.get_legend_handles_labels()
        ax.legend(handles, labels, loc='lower center', bbox_to_anchor=(0.5, 0.835), ncol=5, fontsize=9, frameon=True, framealpha=1)
...
def plot_sparsity(plot_data, baseline_data):
    ...
    _, axes = plt.subplots(1, 3, figsize=(12, 2.5), dpi=300, sharex='col', sharey='row')
    for idx, (ax, sparsity) in enumerate(zip(axes, sparsities)):
        ...
        ax.set_title(f"{int(float(sparsity) * 100)}% of annotations", weight="bold")
        ...
        # Plot the baseline
        plot_baseline(ax, baseline_data[('bcos', 'BCos', 'Input')], markers, colours, score_type="BB-Loc")
        ...
        for loss_fn in ["Energy", "L1"]:
            ...
            if config_key in plot_data:
                ...
                if sorted_pareto_points.size > 0:
                    f_scores, loc_scores = zip(*sorted_pareto_points)
                    ax.scatter(f_scores, loc_scores, label=loss_fn, marker=markers[loss_fn], color=colours[loss_fn], edgecolors='black', s=69, zorder=3)
        ...
        # Add legend
        handles, labels = ax.get_legend_handles_labels()
        ax.legend(handles, labels, loc='lower center', bbox_to_anchor=(0.5, 0.85), ncol=5, fontsize=9, frameon=True, framealpha=1)
...
def plot_domination(ax, baseline_x, baseline_y):
    ...
    ax.fill_between([baseline_x, ax.get_xlim()[1]], baseline_y, ax.get_ylim()[1], color='#B3E6B3', alpha=0.8, zorder=0)
    ax.plot([baseline_x, ax.get_xlim()[1]], [baseline_y, baseline_y], linestyle='--', color='#31852A', linewidth=1, zorder=0)  # Bottom line of dominating section
    ax.plot([baseline_x, baseline_x], [baseline_y, ax.get_ylim()[1]], linestyle='--', color='#31852A', linewidth=1, zorder=0)  # Left line of dominating section
    ax.text(ax.get_xlim()[1], baseline_y, 'Dominating', color='green', va='bottom', ha='right', fontsize=9, rotation=90)

    # Create grey 'Dominated' area in the bottom-left quadrant of the baseline
    ax.fill_between([ax.get_xlim()[0], baseline_x], ax.get_ylim()[0], baseline_y, color='#E3E3E3', alpha=0.8, zorder=0)
    ax.plot([baseline_x, baseline_x], [ax.get_ylim()[0], baseline_y], linestyle='--', color='grey', linewidth=1, zorder=0)  # Right line of dominated section
    ax.plot([ax.get_xlim()[0], baseline_x], [baseline_y, baseline_y], linestyle='--', color='grey', linewidth=1, zorder=0)  # Top line of dominated section
    ax.text(ax.get_xlim()[0], ax.get_ylim()[0], 'Dominated', color='black', va='bottom', ha='left', fontsize=9)


def plot_baseline(ax, baseline_data, markers, colors, score_type):
    baseline_data = baseline_data[0]
    baseline_x, baseline_y = baseline_data["F-Score"] * 100, baseline_data[score_type] * 100
    ax.scatter(baseline_x, baseline_y, label="baseline", marker=markers["baseline"], color=colors["baseline"], edgecolors='black', s=69, zorder=3)
...
def plot_epg_star(plot_data, baseline_data, loss_fn1="Energy", loss_fn2="Energy*"):
    ...
    for loss_fn in [loss_fn1, loss_fn2]:
        ...
        if config_key in plot_data:
            ...
            if sorted_pareto_points.size > 0:
                f_scores, loc_scores = zip(*sorted_pareto_points)
                ax.scatter(f_scores, loc_scores, label=f"{loss_fn}", marker=markers[loss_fn], color=colours[loss_fn], edgecolors='black', s=69, zorder=3)
                ax.plot(f_scores, loc_scores, linestyle='--', color=colours[loss_fn], linewidth=2, zorder=1)

    ax.grid(True)
    handles, labels = ax.get_legend_handles_labels()
    ax.legend(handles, labels, loc='lower center', bbox_to_anchor=(0.5, 0.91), ncol=3, fontsize=9, frameon=True, framealpha=1)
...
def plot_seg_mask(plot_data, baseline_data):
    ...
    colours = {"baseline": "#FFFFFF", "100-BB": "#ED3E63", "9-BB": "#39D095", "9-Seg": "#FECB5B"}
    ...
    for config in ["100-BB", "9-BB", "9-Seg"]:
        ...
        if config_key in plot_data:
            ...
            if sorted_pareto_points.size > 0:
                f_scores, loc_scores = zip(*sorted_pareto_points)
                ax.scatter(f_scores, loc_scores, label=labels[config], marker=markers[config], color=colours[config], edgecolors='black', s=69, zorder=3)
                ax.plot(f_scores, loc_scores, linestyle='--', color=colours[config], linewidth=2, zorder=1)

    ax.grid(True)
    handles, labels = ax.get_legend_handles_labels()
    ax.legend(handles, labels, loc='lower center', bbox_to_anchor=(0.5, 0.91), ncol=4, fontsize=9, frameon=True, framealpha=1)
