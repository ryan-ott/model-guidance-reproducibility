...
def eval_model(model, attributor, loader, num_batches, num_classes, loss_fn, writer=None, epoch=None):
    ...
    return metric_vals, bb_metric.fractions, bb_metric.bbox_sizes
...
def main(args):
    ...
    metrics, epg_fractions, bbsizes = eval_model(model_activator, eval_attributor, test_loader,
                                num_test_batches, num_classes, loss_fn)
    result_dict[args.checkpoint_path] = metrics
    epg_fractions_cpu = [tensor.detach().cpu().numpy() for tensor in epg_fractions]
    #bbsizes_cpu = [tensor.detach().cpu().numpy() for tensor in bbsizes]
    df = pd.DataFrame({
        "epg_fraction": epg_fractions_cpu,
        "bbsize": bbsizes
    })
    df.to_csv('result_jsons_epg_bbx/epg_bbsize.csv', index=False)
    print(f'Saving final results in result_jsons_epg_bbx/{args.model_backbone}_{args.attribution_method}_{args.layer}_{args.localization_loss_fn}_{args.mode}_{args.percent}.json ...')
    with open(f'result_jsons_epg_bbx/{args.model_backbone}_{args.attribution_method}_{args.layer}_{args.localization_loss_fn}_epg_bbsize.json', 'w') as f:
...
parser.add_argument("--mode", type=str, default=None, choices=[None, "dilation", "sparse"], help="str to add to path if dilation/sparse")
parser.add_argument("--percent", type=float, default=None, help="str to add to path if dilation/sparse")
