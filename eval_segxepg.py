...
def eval_model(model, attributor, loader, num_batches, num_classes, loss_fn, writer=None, epoch=None):
    ...
    if args.mode =='bb':
        ...
        f1_metric = metrics.MultiLabelMetrics(
            num_classes=num_classes, threshold=0.0)
        bb_metric = metrics.SegmentationEnergyMultiple()
        iou_metric = metrics.SegmentationIoUMultiple()
    total_loss = 0
...
def main(args):
    ...
    print(f'Saving final results in result_jsons/segxexp_{args.model_backbone}_{args.attribution_method}_{args.layer}_{args.localization_loss_fn}.json ...')
    with open(f'result_json/segxepg_{args.model_backbone}_{args.attribution_method}_{args.layer}_{args.localization_loss_fn}.json', 'w') as f:
