...
def main(args):
    ...
    out_name = model_prefix + "_" + optimize_explanation_str + "_attr" + str(args.attribution_method) + "_locloss" + str(args.localization_loss_fn) + "_orig" + orig_name + "_resnet50" + "_lr" + str(
        args.learning_rate) + "_sll" + str(args.localization_loss_lambda) + "_layer" + str(args.layer)+'_seg'
    ...
    train_data = datasets.VOCDetectSegParsed(
        root=root, image_set="train_seg", transform=transformer, annotated_fraction=args.annotated_fraction)
    ...
    train_loader = torch.utils.data.DataLoader(
        train_data, batch_size=args.train_batch_size, shuffle=True, num_workers=4, collate_fn=datasets.VOCDetectSegParsed.collate_fn)
            ...
            if args.optimize_explanations:
                ...
                for img_idx in range(len(train_X)):
                    ...
                    bb_list = utils.filter_bbs(
                        train_bbs[img_idx], gt_classes[img_idx], seg=True)
                    if args.box_dilation_percentage > 0:
...
parser.add_argument("--localization_loss_fn", type=str, default=None,
                    choices=["Energy", "L1", "RRR", "PPCE","Energy_mod", "Energy_seg"], help="Localization loss function to use.")
